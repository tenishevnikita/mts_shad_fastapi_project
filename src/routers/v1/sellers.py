...
from src.service import SellerService
...
async def create_seller(seller: IncomingSeller, session: DBSession):
    return await SellerService.create_seller(seller=seller, session=session)


@sellers_router.get("/{seller_id}", response_model=ReturnedSellerWithBooks)
async def get_seller(seller_id: int, session: DBSession):
    return await SellerService.get_seller(seller_id=seller_id, session=session)


@sellers_router.get("/", response_model=ReturnedAllSellers)
async def get_all_sellers(session: DBSession):
    return await SellerService.get_all_sellers(session=session)


@sellers_router.delete("/{seller_id}")
async def delete_seller(seller_id: int, session: DBSession):
    return await SellerService.delete_seller(seller_id=seller_id, session=session)


@sellers_router.put("/{seller_id}", response_model=ReturnedSeller)
async def update_seller(seller_id: int, new_data: UpdatedSeller, session: DBSession):
    return await SellerService.update_seller(seller_id=seller_id, new_data=new_data, session=session)
